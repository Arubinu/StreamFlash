...
try:
	...
	class fcntl():
		...
		def ioctl( self, fd, op, arg = 0, mutable_flag = True ):
			if mutable_flag:
				return 0
			else:
				return ""
		...
		def lockf( self, fd, operation, length = 0, start = 0, whence = 0 ):
			return
...
class TwitchChatStream( object ):
	...
	def _logged_in_successful( data ):
		...
		"""
		return ( not re.match( r'^:(testserver\.local|tmi\.twitch\.tv) NOTICE \* :(Login unsuccessful|Error logging in)*$', data.strip() ) )
	...
	def _check_has_channel( data ):
		...
		"""
		return ( re.findall( r'^:[a-zA-Z0-9_]+\![a-zA-Z0-9_]+@[a-zA-Z0-9_]+' r'\.tmi\.twitch\.tv ' r'JOIN #([a-zA-Z0-9_]+)$', data ) )
	...
	def _parse_message( self, data ):
		...
		if TwitchChatStream._check_has_channel( data ):
			self.current_channel = TwitchChatStream._check_has_channel( data )[ 0 ]

		if TwitchChatStream._check_has_message( data ):
			return {
				'channel': re.findall( r'^:.+![a-zA-Z0-9_]+' r'@[a-zA-Z0-9_]+' r'.+ ' r'PRIVMSG (.*?) :', data )[ 0 ],
		...
		while True:
			...
			try:
				...
				if msg and msg.strip():
					if self.verbose:
						print( msg )
					rec = [ self._parse_message( line ) for line in filter( None, msg.split( '\r\n' ) ) ]
